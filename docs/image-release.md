...
A `publish=true` dispatch from `main` first requires a successful `CI` push run for the **exact selected main SHA** (not a green run from another commit), before its first registry write. After its local runtime and scan gates pass, it pushes a run/attempt-unique staging ref and captures the digest from that push output. It fetches the remote immutable manifest at that digest and fails unless its config digest equals the scanned local image config. Only then does it promote that immutable digest to the SHA tag without rebuilding; no archive transfer occurs after scanning. Publication attaches SLSA build provenance and the bounded package-level SPDX document to that exact digest.
