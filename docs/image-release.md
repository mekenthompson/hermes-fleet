...
A `publish=false` dispatch runs the same read-only preflight bake and evidence gates as a pull request; it has no package-write permission and cannot write a registry reference. A `publish=true` dispatch from `main` first requires a successful `CI` push run for the **exact selected main SHA** (not a green run from another commit), before its first registry write. After its local runtime and scan gates pass, it pushes a run/attempt-unique staging ref and captures the digest from Docker's `tag: digest: sha256:… size: N` push summary. It fetches the remote immutable manifest at that digest and fails unless its config digest equals the scanned local image config. Only then does it promote that immutable digest to the SHA tag without rebuilding, explicitly preserving its single-manifest form; it reads that SHA tag back and fails unless it resolves to the staged digest. No archive transfer occurs after scanning. Publication attaches SLSA build provenance and the bounded package-level SPDX document to that exact digest.
...
Only `main` may enter `fleet-image-publish`. There is no manual reviewer prerequisite: the exact-SHA CI proof, runtime checks, full SPDX, compact SPDX, Trivy result, VEX revalidation, remote immutable-manifest config binding, and attestations are mandatory and fail closed. Publication still does not authorize rollout.
