...
- **Entrust work × configured Linear integration:** observe the same owner and material milestones, delay delivery, and verify catch-up without relaunch. When Linear is not configured, this optional delivery path is not claimed. Protects `built-published-deployed-are-different`. To validate.
