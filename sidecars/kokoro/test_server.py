...
Run: python3 -m unittest discover -s sidecars/kokoro -p 'test_*.py'
"""
...
class WatchdogTests(unittest.TestCase):
    def test_recovery_schedules_exactly_one_hard_exit(self):
        from unittest.mock import patch
        with patch.object(server, '_wedged', False), patch.object(server.threading, 'Timer') as timer, patch.object(server.os, '_exit') as terminate:
            server._request_recovery('tts')
            self.assertTrue(server._is_wedged())
            server._request_recovery('stt')
            timer.assert_called_once()
            timer.return_value.start.assert_called_once()
            timer.call_args.args[1]()
            terminate.assert_called_once_with(70)
...
class _FakePool:
    ...
    def submit(self, _fn, *args, timeout):  # noqa: ANN001, ANN201
