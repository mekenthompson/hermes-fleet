...
# kept (they disagreed above 999 and neither did decimals). Its default
...
def _dispatch_verbatim(content: str) -> str:
    ...
    ("voice, config, dir" rather than one mushed word).
    """
    words = content.split()
    out: List[str] = []
    for word in words:
...
# `~` is deliberately NOT in this set: it still holds meaning at this point
...
def _month_abbrev(text: str) -> str:
    ...
    month; requiring an adjacent day number is what keeps "Mario" and "Sept"
...
def _units(text: str) -> str:
    # A range shares its unit: only the second number keeps it, so without
