#!/usr/bin/env python3
"""
Local GPU speech sidecar — STT (PR-B2) + TTS (PR-C1).
...
lexicon holds it as a plain string /lˈid/ (not a POS-keyed entry), so
...
Design constraints (reviewed):
  * Auth — every /stt and /tts call must carry the X-Voice-Token shared
    secret (injected by the host overlay). A missing/wrong
...
from worker_lane import WorkLane, LaneBusy
...
# Watch the physical operation in each lane, independently of HTTP timeouts.
# A full/expired queue never counts as a native-call wedge. A physical call
# exceeding MAX_TIMEOUTS times its request timeout starts bounded process exit;
# Docker's restart policy can then replace the process and its GPU context.
VOICE_WATCHDOG_MAX_TIMEOUTS = max(1, int(os.environ.get("VOICE_WATCHDOG_MAX_TIMEOUTS", "3")))
VOICE_WATCHDOG_RESTART_GRACE_S = float(os.environ.get("VOICE_WATCHDOG_RESTART_GRACE_S", "5"))
import math
if not math.isfinite(VOICE_WATCHDOG_RESTART_GRACE_S) or not 0 < VOICE_WATCHDOG_RESTART_GRACE_S <= 60:
    raise ValueError("VOICE_WATCHDOG_RESTART_GRACE_S must be within (0, 60]")
_watchdog_lock = threading.Lock()
_wedged = False


def _request_recovery(lane: str) -> None:
    """Exit despite blocked native threads; health alone cannot restart Docker."""
    global _wedged
    with _watchdog_lock:
        if _wedged:
            return
        _wedged = True
        # Hard exit is intentional: normal Python shutdown joins worker threads
        # and could wait forever on the very native call being recovered.
        timer = threading.Timer(VOICE_WATCHDOG_RESTART_GRACE_S, lambda: os._exit(70))
        timer.daemon = True
        timer.start()
    _log(f"watchdog: {lane} physical call stalled; process exit scheduled")
...
_pool = WorkLane("stt", workers=CONCURRENCY, pending=max(1, CONCURRENCY * 2),
                 on_stalled=_request_recovery, stall_multiplier=VOICE_WATCHDOG_MAX_TIMEOUTS)
...
_tts_pool = WorkLane("tts", workers=TTS_CONCURRENCY, pending=max(1, TTS_CONCURRENCY * 2),
                     on_stalled=_request_recovery, stall_multiplier=VOICE_WATCHDOG_MAX_TIMEOUTS)
...
def _split_phonemes(phonemes: str, max_phonemes: int) -> tuple[list[str], int]:
    ...
    tail or (usually) raises; this keeps the tail into the next chunk instead.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 (stdlib naming)
        ...
        # A physical-call wedge stays unhealthy until the scheduled hard exit.
        # Docker restarts the exited process; healthcheck failures alone do not.
        if _is_wedged():
            self._send_json(
                503,
                {
    ...
    def _handle_stt(self) -> None:
        if not self._authed():
            return
        if _is_wedged():
            self._send_json(503, {"ok": False, "reason": "restarting"})
            return
        ...
        try:
            future = _pool.submit(_run_inference, wav, language, timeout=REQUEST_TIMEOUT_S)
        except LaneBusy:
            self._send_json(503, {"ok": False, "reason": "busy"})
            return
        try:
            result = future.result(timeout=REQUEST_TIMEOUT_S)
        except FutureTimeout:
            # Cancel queued work; active native work retains its physical watchdog.
            future.cancel()
    ...
    def _handle_tts(self) -> None:
        ...
        try:
            future = _tts_pool.submit(_run_synthesis, text, voice, speed, timeout=TTS_REQUEST_TIMEOUT_S)
        except LaneBusy:
            self._send_json(503, {"ok": False, "reason": "busy"})
            return
        try:
