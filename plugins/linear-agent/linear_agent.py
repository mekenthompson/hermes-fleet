...
def _session_title(response: str) -> str:
    """Linear session summary: 1-255 chars, no line breaks or NUL bytes."""
    redacted = _redact_multiline_secret_values(response)
    text = " ".join(redacted.split()).replace("\0", "")
    if not text:
        text = "Completed."
    if len(text) > 255:
        return text[:254] + "…"
...
class LinearWorker:
    ...
    def __init__(
        ...
        *,
        ...
        parent_lookup: Callable[[str], str] | None = None,
        parent_deliver: Callable[[str, str], bool] | None = None,
    ) -> None:
        ...
        self.parent_deliver = parent_deliver
    ...
    def _queue_terminal_issue(
        ...
        *,
        ...
        if not job.issue_id:
            return
        if has_follow_up:
            # A queued follow-up is not a principal decision. Do not park in the waiting state.
            return
    ...
    def add_delivery(self, delivery_id: str, payload: bytes) -> None:
        ...
        try:
            ...
            if (event.get("type") == "AgentSessionEvent" if isinstance(event, dict) else False) and (
                ...
                and activity.get("signal") in STOP_ACTIVITY_SIGNALS
            ):
                control_priority = 1
    ...
    def import_from_ingress_once(self, ingress_database: Path) -> bool:
        ...
        with closing(sqlite3.connect(ingress_database, isolation_level=None, timeout=5.0)) as ingress:
            ...
            row = ingress.execute(
                ...
                "AND json_extract(CAST(payload AS TEXT), '$.agentActivity.signal') = 'stop' "
    ...
    def _standard_job_from_payload(
        ...
        if is_created:
            ...
            if signal is not None:
                ...
                if not isinstance(signal, str) or signal not in LINEAR_ACTIVITY_SIGNALS:
                    raise ValueError("unsupported Agent Activity signal")
                if signal in STOP_ACTIVITY_SIGNALS:
            ...
            if not prompt:
                if isinstance(content, dict):
                    ...
                    prompt = content
            if not prompt and signal in FOLLOW_UP_ACTIVITY_SIGNALS:
                prompt = "Continue."
        else:
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            self._insert_outbox(
                ...
                "session_update",
                job.linear_session_id,
                json.dumps({"summary": _session_title(response)}, separators=(",", ":")),
            )
            if job.issue_id:
                self._insert_outbox(
                    conn,
    ...
    def _enqueue_parent_followup(self, job: QueuedLinearJob) -> None:
        ...
        with self._connect() as conn:
            ...
            parent_id = resolve_parent_issue_id(
                bytes(row[0]) if row is not None else b"",
        ...
        self._deliver_parent_followups()

    def _deliver_parent_followups(self) -> None:
        """Inject queued trusted summaries into the owning Hermes session."""
        if self.parent_deliver is None:
            return
        queue = ParentFollowupQueue(self.database)
        for _ in range(self.limits.max_concurrent_jobs):
            result = queue.deliver_queued(self.parent_deliver)
    ...
    def fail_job(self, job: QueuedLinearJob, response: str | None = None) -> None:
        if not isinstance(response, str) or not response.strip() or "\0" in response:
